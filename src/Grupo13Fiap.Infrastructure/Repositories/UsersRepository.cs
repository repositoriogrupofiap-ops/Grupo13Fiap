...
public class UsersRepository(DBContextGrupo13Fiap context) : GenericRepository<User>(context), IUsersRepository
{
    ...
    public async Task<User?> GetWithLibraryAsync(Guid id, CancellationToken cancellationToken = default)
        => await Query()
            .Include(u => u.Library)
